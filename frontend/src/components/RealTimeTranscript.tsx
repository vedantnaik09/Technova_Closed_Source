...
import { firestore, database, storage } from "../lib/firebase";
...
const RealTimeTranscript: React.FC<{ callId: string | undefined; remoteStreams: (MediaStream | null)[] }> = ({ callId, remoteStreams }) => {
  ...
  const finalTextsGlobal = useRef<Record<string, string> | undefined>(null);
  ...
  const mediaRecorderforFile = useRef<MediaRecorder>(null);
  ...
  const run = async () => {
    ...
    if (role) {
      try {
        await callDocHost.set({ role }, { merge: true });
        ...
        if (status !== "RECORDING") {
          ...
          newSocket.onclose = (event) => {
            console.log("socket connection closed 2 ", event);
            socket.current = null;
          ...
          newSocket.onopen = async () => {
            ...
            recorder.onstop = () => {
              ...
              uploadBytes(audioFileRef, audioBlob).then((snapshot) => console.log("Uploaded a blob or file!", snapshot));
              getDownloadURL(audioFileRef)
                ...
                .catch((error) => console.log("Error while uploading audio file ", error));
            };
